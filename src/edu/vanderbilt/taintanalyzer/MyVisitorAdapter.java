...
public class MyVisitorAdapter<A> implements VoidVisitor<A> {
	...
	public void visit(LambdaExpr n, A arg) {
		if (n.getParentNode().isPresent())
		{
			...
			if ((Parent_ instanceof MethodCallExpr)  && (((MethodCallExpr) Parent_).getNameAsString().equals("forEach")))
			{
				Integer lc = this.loop_counter.peek()+1;
				String pn = new String(this.parent_loop_label.peek() +"_L"+ lc);
				this.loop_counter.pop();
	...
	{
    	Expression exp = new BinaryExpr(new NameExpr("MultiTainter.getTaint(" + var_name + ")" ), new NullLiteralExpr(), com.github.javaparser.ast.expr.BinaryExpr.Operator.NOT_EQUALS);        	
    	String S1 = new String("TaintEntry T_"+ var_name +" = new TaintEntry(\""+ parent_loop_name + "\", \"" + var_name +"\", MultiTainter.getTaint("+ var_name +").getDependencies())");
