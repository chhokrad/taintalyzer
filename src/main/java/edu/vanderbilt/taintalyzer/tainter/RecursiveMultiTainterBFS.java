package edu.vanderbilt.taintalyzer.tainter;
...
import edu.vanderbilt.taintalyzer.utility.LevelObjPair;
import edu.vanderbilt.taintalyzer.utility.LevelObjPairComparator;
import edu.vanderbilt.taintalyzer.utility.ResultsMap;
			...
			for (Field f : obj.getClass().getDeclaredFields()) {
				...
				if (ClassUtils.isPrimitiveOrWrapper(f.getType())) {
					if (!Modifier.isFinal(f.getModifiers())) {
						...
						System.out.println("Skipping tainting a Final Field : " + f.getType() + " " + f.getName()
								+ " in " + obj.getClass().getName());
				} else if ((f.get(obj)).getClass().isArray() && this.isPrimitiveArray(f.get(obj))
						&& !this.isGreaterThanOneDimension(f.get(obj)))
					f.set(obj, this.taintPrimitiveArrayWreturnfiltered(f.get(obj)));
				else if ((f.get(obj)).getClass().isArray() && this.isPrimitiveArray(f.get(obj))
						&& this.isGreaterThanOneDimension(f.get(obj)))
					this.taintPrimitiveArrayfiltered(f.get(obj));
				else
					this.myQueue.add(new LevelObjPair(p.getLevel() + 1, f.get(obj)));
			}
		}
